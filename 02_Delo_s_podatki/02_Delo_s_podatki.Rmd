...
## Branje podatkov iz Excela {.smaller}
...
```
- Dodajmo rezultat v razpredelnico kot nov stolpec (podobno kot formule pri Excelu)
```r
> podatki$Št_prijateljev_na_leto <- podatki$Št_prijateljev/podatki$Starost
> View(podatki)
```
- <img src="viri/nov_stolpec.png" width="400px" float=center>
- namesto ```podatki$ime``` bi lahko pisali ```podatki["ime"]```

## Knjižnica ```dplyr``` {.smaller}
...
- Če ne izpisujemo tabel, vrstni red stolpcev ni važen.
...
```r
> podatki <- podatki %>% rename(st_prijateljev=Št_prijateljev) 
```
...
## Shranjevanje v Excelovo datoteko {.smaller}
...
## Razmejeni tekstovni format (CSV) {.smaller}
...
- Zapis razpredelnice v CSV datoteko v evropskem formatu
```r
> podatki %>% write.csv2("viri/primer2.csv",fileEncoding = "utf8")
```
- Branje:
```r
